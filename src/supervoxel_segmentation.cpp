...
pcl::PointCloud<pcl::PointXYZRGB>::Ptr SupervoxelSegmentation::publishSupervoxel(
    ...
    for (std::map<uint32_t, pcl::Supervoxel<PointT>::Ptr >::const_iterator
      ...
      int rand1 = rand()  % 255 + 0;
      int rand2 = rand()  % 255 + 0;
      int rand3 = rand()  % 255 + 0;
      for (size_t i = 0; i < super_voxel_cloud->size(); i++) {
        indices.indices.push_back(i + output->points.size());
        uint32_t rgb = ((uint32_t)rand1 << 16 | (uint32_t)rand2 << 8 | (uint32_t)rand3);
        super_voxel_cloud->points.at(i).rgb = *reinterpret_cast<float*>(&rgb);
      }
